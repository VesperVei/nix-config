...
Reusable, symlink-first agent resources shared across AI CLI tools.
...
- `opencode-permission-tmpl.json`: reference permission template for OpenCode.
...
Use `install-cli.md` and `install-skills.md` as snippet libraries:
...
- Do not manage API keys, provider tokens, or MCP tokens in tracked files.
